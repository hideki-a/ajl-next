 ...
 * @author Rahul Kumar (https://medium.com/@im_rahul/focus-trapping-looping-b3ee658e5177)
 */
export default function loopFocus(config) {
    if (!config) {
        throw new Error('Could not initialize focus-trapping - Config Missing');
    }
    const { el } = config;
    // const { escCallback } = config;
    ...
    if (focusableElements.length > 0) {
        ...
        const elementToFocus = focusElement ? focusElement : firstFocusableEl;
